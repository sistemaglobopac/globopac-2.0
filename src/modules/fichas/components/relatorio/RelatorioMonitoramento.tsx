...
function CampoSimples({ label, tipo, valor }: { label: string; tipo: string; valor: unknown }) {
...
/** "Dados Coletados em Campo" de UM registro: itera schema_campos na ordem real da ficha
 * (fonte de verdade estruturada — sem as heurísticas de regex do v1) e delega os 7 widgets
 * "Especial SIF" para CamposEspeciaisRelatorio. */
function DadosColetados({ dadosDinamicos, campos }: { dadosDinamicos: Record<string, unknown>; campos: CampoTemplate[] }) {
  if (campos.length === 0) {
